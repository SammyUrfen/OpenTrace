...
- `sessions.py` / `terminals.py` / `runs.py` / `run_views.py` — pydantic models, CRUD,
  ...
  Phase-6 `profile` and `flamegraph`).
...
- `rules/engine.py` — 18 anomaly rules (file I/O, memory growth/spike, CPU/spin/
  ...
  Real-time alerts (FD>200 / memory-spike / sustained-CPU) are emitted live from
  `orchestrator._live_detect` on each metric sample (SSE `anomaly_alert`).
...
- `main.js` — spawns the backend (`uvicorn`), waits on `/health`, creates the
  ...
  `OPENTRACE_BACKEND_URL` points it at an already-running backend (skips spawning,
  for dev/testing); `OPENTRACE_WIN=WxH` sizes the window.
